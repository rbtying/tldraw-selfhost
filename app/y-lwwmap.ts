...
type ChangeLogEntry<T> = {
  // represents a change of an LWWMap entry
  Key: string;
  Value?: T;
  Timestamp: number;
};
...
type ChangeEvent<T> = {
  change: (
    ...
    >,
    event: Map<string, ChangeLogEntry<T>>
  ) => void;
};
...
export class LWWMap<
  T extends object | boolean | Array<T> | string | number | null | Uint8Array
> extends ObservableV2<ChangeEvent<T>> {
  ...
  public constructor(
    ...
    this.sharedArray.observe((Event: any, Transaction: any) =>
      this._updateOnChange(Event, Transaction)
    );
  }
  ...
  public delete(Key: string): boolean {
    if (this.localMap.has(Key)) {
      // ignore deletions of missing entries
      this.sharedArray.doc.transact(() => {
  ...
  public forEach(
    Callback: (Value: T, Key: string, Map: LWWMap<T>) => void,
    thisArg?: any
  ): void {
    this.localMap.forEach((loggedEntry: ChangeLogEntry<T>, Key: string) => {
      if ("Value" in loggedEntry) {
        // ignore entries describing deletions
  ...
  protected _ChangesCollide(
    firstChange: ChangeLogEntry<T>,
    secondChange: ChangeLogEntry<T>
  ): boolean {
    return (
  ...
  protected _initializeMap(): void {
    ...
    this.sharedArray.doc.transact(() => {
      for (let i = ChangeLog.length - 1; i >= 0; i--) {
        ...
        if ("Value" in loggedChange) {
          // "loggedChange" defines existing entry
          switch (true) {
            case !KeyIsKnown:
          ...
          // "loggedChange" defines a deleted entry
          switch (true) {
            case !KeyIsKnown:
  ...
  protected _updateOnChange(Event: any, Transaction: any): void {
    ...
    const UpdateLog: ChangeLogEntry<T>[] = Array.from(Event.changes.added)
      .map((addedContent: any) => addedContent.content.getContent())
    ...
    try {
      ...
      // refresh affected entries to keep them consistent
    ...
    if (EventLog.size > 0) {
      ...
      this.sharedArray.doc.transact(() => {
        for (let i = ChangeLog.length - 1; i >= 0; i--) {
          ...
          if (
            EventLog.has(Key) &&
            EventLog.get(Key).newValue !== loggedChange.Value
          ) {
            this.sharedArray.delete(i);
          }
        }
  ...
  protected _removeAnyObsoleteDeletions(): void {
    let RetentionTimestamp =
      Date.now() * TimestampFactor - this.RetentionPeriod;
